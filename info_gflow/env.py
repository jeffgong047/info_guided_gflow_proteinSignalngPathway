...
class GFlowNetDAGEnv(gym.vector.VectorEnv):
    def __init__(
        ...
        """
        #scorers['scorer'] is bayesian scorer by default
        for scorer_name, scorer in scorers.items():
            setattr(self, scorer_name, scorer)
        self.num_workers = num_workers
        self.num_variables = scorers['scorer'].num_variables
    ...
    def step(self, actions):
        ...
        logging.info(f'adding edge from {sources} to {targets}')
        # if dones:
        #     logging.info(f'current trajectory is doneeeeeeeeeeeeeeeeeeeeeeeee!')
    ...
    def local_scores_async(self, sources, targets):
        ...
        for i, (source, target) in enumerate(zip(sources, targets)):
            if source == self.num_variables:
                ...
                indices_after = list(indices)  # indices_after  = nodes that are parent of the target variable
            ...
            keys.append(key)
       # print('keys', keys, 'local_cache', local_cache, 'queued_data', queued_data)
    ...
    def local_scores_wait(self, keys, local_cache, data):
        ...
        for target, key_tm1, key_t in keys:
            if target is None:
             ...
             #   breakpoint()
            #    print('target: ',target, 'indices after: ', key_t, 'indices before: ', key_tm1)
            #    print('original local score: ', self.local_scores[(target, key_tm1)], 'new local score: ',self.local_scores[(target, key_t)])
                if self.info_constraint_scorer is not None:
                    soft_info_score = self.info_constraint_scorer.soft_info_constraint([key_t[0],target],self._state['adjacency'])
           #     print('soft info constraint: ', soft_info_score)
                delta_scores.append(
                    self.local_scores[(target, key_t)]
                    - self.local_scores[(target, key_tm1)]
                    + soft_info_score)
                logging.info(f'changes of local score:{self.local_scores[(target, key_t)]-self.local_scores[(target, key_tm1)]}')
