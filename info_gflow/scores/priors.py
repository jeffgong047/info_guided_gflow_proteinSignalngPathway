...
class informationConstrainPrior(BasePrior):
    @property
    def log_prior(self):
        if self._log_prior is None:
            all_parents = np.arrange(self.num_variables)
            prior_score = multi_info_score(*args,all_parents)
            breakpoint() # we might revise the prior score over here
            self._log_prior= (
                - gammaln(self.num_variables + 1)
                + gammaln(self.num_variables - all_parents + 1)
                + gammaln(all_parents + 1)
