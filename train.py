...
from dag_gflownet.utils.factories import get_scorers
...
def main(args):
    logging.basicConfig(filename='erdos_renyi.log', level=logging.DEBUG)
    os.environ['XLA_PYTHON_CLIENT_PREALLOCATE'] = 'false'
    rng = default_rng(args.seed)
    key = jax.random.PRNGKey(args.seed)
    key, subkey = jax.random.split(key)
    # Create the environment
    scorers, data, graph = get_scorers(args, rng=rng)
    env = GFlowNetDAGEnv(
    ...
    with trange(args.prefill + args.num_iterations, desc='Training') as pbar:
        for iteration in pbar:
            logging.info(f'iteration number {iter}')
            iter+=1
...
if __name__ == '__main__':
    ...
    environment.add_argument('--info_constraint', type =bool, default=False)
    ...
    #Non-linear gaussian
    nonlinear_gaussian = subparsers.add_parser('nonlinear_gaussian')
    nonlinear_gaussian.add_argument('--num_variables', type=int, required=False, help='Number of variables')
    nonlinear_gaussian.add_argument('--num_samples', type= int, required=False, help='Number of samples')

    synthetic_signaling = subparsers.add_parser('synthetic_signaling')
    synthetic_signaling.add_argument('--num_variables', type=int, required=True,
                                       help = 'Number of variables')
    synthetic_signaling.add_argument('--data_instance_index', type= int, default=1,
                                       help='index of the synthetic data instance')
    synthetic_signaling.add_argument('--num_samples', type=int, default=875)

    # Flow cytometry data (Sachs) with observational data
    sachs_continuous = subparsers.add_parser('sachs_continuous')
    do_structure = subparsers.add_parser('do_structure')
    synthetic15_3_observational = subparsers.add_parser('synthetic15_3_observational')
    hematopoietic = subparsers.add_parser('hematopoietic')
    demo = subparsers.add_parser('demo')
    # Flow cytometry data (Sachs) with interventional data
    sachs_intervention = subparsers.add_parser('sachs_interventional')
    sachs_allConditioned = subparsers.add_parser('sachs_allConditioned')
    demo_interven = subparsers.add_parser('demo_interven')
